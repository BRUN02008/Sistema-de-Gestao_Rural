...
function valor(v: any, fallback = "________________") {
...
function formatarArea(produtor: any) {
  return valor(produtor.areaTotal, "________________");
}

function formatarProducaoPropriedade(produtor: any) {
  const itens: string[] = [];

  if (produtor.culturaPrincipal) itens.push(produtor.culturaPrincipal);

  if (Array.isArray(produtor.producaoFlorestal)) {
    produtor.producaoFlorestal.forEach((item: any) => {
      if (item?.produto) itens.push(item.produto);
    });
  }

  if (Array.isArray(produtor.pecuaria)) {
    produtor.pecuaria.forEach((item: any) => {
      if (item?.tipo) itens.push(item.tipo);
    });
  }

  if (Array.isArray(produtor.abelhas)) {
    produtor.abelhas.forEach((item: any) => {
      if (item?.tipo) itens.push(item.tipo);
    });
  }

  if (itens.length === 0 && produtor.outrasProducoes) {
    itens.push(produtor.outrasProducoes);
  }
