...
function formatarEspecies(especies: any[]) {
  if (!Array.isArray(especies) || especies.length === 0) {
    return "0 kg de __________________";
  }

  const lista = especies
    .filter((item) => item?.nome || item?.kg)
    .map(
      (item) =>
        `${item?.kg || "0"} kg de ${item?.nome || "__________________"}`,
    );
...
function formatarSubsistencia(produtor: any) {
  const itens: string[] = [];
...
export default function DeclaracaoPescadorOficial({
  ...
  const especiesTexto = formatarEspecies(produtor.especies || []);
