...
export default function CronogramaSemanalMelhorado() {
  ...
  const [produtoresFiltrados, setProdutoresFiltrados] = useState<Produtor[]>([]);
  ...
  useEffect(() => {
    if (buscaProdutor.length > 0) {
      const filtrados = produtores
        ...
        .slice(0, 5);
      setProdutoresFiltrados(filtrados);
      setMostrarSugestoes(true);
    } else {
      setProdutoresFiltrados([]);
  ...
  const carregarDados = () => {
    ...
    if (produtoresStorage) {
      setProdutores(JSON.parse(produtoresStorage));
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* HEADER */}
      <div className="bg-gradient-to-r from-pink-600 to-pink-700 rounded-xl p-6 shadow-lg text-white">
        ...
        <div className="bg-card rounded-xl border-2 border-pink-500/20 p-6 shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      tipoAtividade: e.target.value as any,
                    })
                  }
                  className="w-full px-4 py-3 rounded-lg border-2 border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-pink-500"
                >
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, turno: turno.value as any })
                    }
            ...
            <div className="flex gap-3">
              <button
                type="submit"
                className="flex-1 px-6 py-3 bg-gradient-to-r from-pink-600 to-pink-700 text-white rounded-lg hover:from-pink-700 hover:to-pink-800 transition-all shadow-lg font-semibold"
              >
