...
function listaTemDados(lista: any[] = []) {
  return Array.isArray(lista) && lista.some(temDados);
}

function objetoTemDados(obj: any) {
  if (!obj) return false;
...
function produtorTemCategoria(produtor: any, categoria: string) {
  return (produtor.atividades || []).some(
    (a: any) => a.categoria === categoria,
  );
}

function produtorTemSubtipo(produtor: any, subtipo: string) {
  return (produtor.atividades || []).some((a: any) =>
...
function renderTabelaPiscicultura(lista: any[] = []) {
  const itensValidos = lista.filter(temDados);
  if (!itensValidos.length) return null;
