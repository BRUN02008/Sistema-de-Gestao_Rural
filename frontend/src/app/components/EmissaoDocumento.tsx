...
export default function EmissaoDocumento() {
  ...
  const [produtorComAtendimento, setProdutorComAtendimento] =
    useState<Produtor | null>(null);
  ...
  useEffect(() => {
    ...
    if (stored) {
      setProdutores(JSON.parse(stored));
    }
  }, []);

  // Carregar dados de atendimento quando um produtor é selecionado
  useEffect(() => {
    if (!selectedProdutor) {
      setProdutorComAtendimento(null);
      return;
    }

    const atendimentos = JSON.parse(localStorage.getItem("atendimentos") || "[]");

    // Buscar o atendimento mais recente do produtor
    const atendimentoDoProdutor = atendimentos.find(
      (a: any) => a.produtorId === selectedProdutor.id || a.produtorCpf === selectedProdutor.cpf
    );

    // Mesclar dados do produtor com dados do atendimento
    if (atendimentoDoProdutor && atendimentoDoProdutor.dados) {
      setProdutorComAtendimento({
        ...selectedProdutor,
      ...
      setProdutorComAtendimento(selectedProdutor);
    }
  }, [selectedProdutor]);
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `${documentType}_${selectedProdutor?.nome || "documento"}`,
  });
