...
export default function ValidadorCpfTrimestre({
  ...
  const verificarCpf = (cpfValue: string) => {
    ...
    const produtorEncontrado = produtores.find(
      (p: Produtor) => p.cpf.replace(/\D/g, "") === cpfLimpo
    );

    if (produtorEncontrado) {
      setProdutorExistente(produtorEncontrado);
      setTrimestre(produtorEncontrado.trimestre || "1");
      setAno(produtorEncontrado.anoTrimestre || new Date().getFullYear().toString());
  ...
  return (
    <div className="space-y-4">
      ...
      {!produtorExistente && cpf && cpf.length >= 11 && !verificando && (
