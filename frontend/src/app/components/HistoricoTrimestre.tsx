...
export default function HistoricoTrimestreComponent() {
  ...
  const [historico, setHistorico] = useState<HistoricoTrimestre[]>([]);
  ...
  const carregarDados = () => {
    const produtoresStorage = JSON.parse(localStorage.getItem("produtores") || "[]");
    setProdutores(produtoresStorage);

    const historicoStorage = JSON.parse(localStorage.getItem("historicoTrimestres") || "[]");
    setHistorico(historicoStorage);
  };

  const buscarProdutorPorCpf = () => {
    const produtor = produtores.find(p => p.cpf === buscaCpf);
    if (produtor) {
  ...
  const historicoDoProdutor = historico.filter(
    h => h.produtorCpf === produtorSelecionado?.cpf
  ).sort((a, b) => {
  ...
  const compararDados = (atual: any, anterior: any) => {
        ...
        <div className="bg-card rounded-xl border border-border p-6 shadow-sm no-print">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-purple-600 to-purple-700 text-white rounded-lg hover:from-purple-700 hover:to-purple-800 transition-all shadow-lg"
            >
